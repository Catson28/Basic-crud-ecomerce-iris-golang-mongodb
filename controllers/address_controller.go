...
func (g *AddressController) Get() (response datamodels.Response) {
	return g.Service.List(nil)
}

func (g *AddressController) GetBy(id string) (response datamodels.Response) {
	return g.Service.GetByID(id)
}

func (g *AddressController) GetName(name string) (response datamodels.Response) {
	return g.Service.GetByName(name)
}

func (g *AddressController) DeleteBy(id string) (response datamodels.Response) {
	return g.Service.DeleteByID(id)
}

func (g *AddressController) PutBy(id string) (response datamodels.Response) {
	address := datamodels.Address{}
	err := g.Ctx.ReadJSON(&address)
	if err != nil {
		response.Code = 40001
		response.Msg = fmt.Sprintf("参数解析失败：%v", err)
	}
	address.ID = bson.ObjectIdHex(id)
	return g.Service.Save(address)
}

func (g *AddressController) Post() (response datamodels.Response) {
	address := datamodels.Address{}
	err := g.Ctx.ReadJSON(&address)
	...
	if err != nil {
		...
		if errs, ok := err.(validator.ValidationErrors); ok {
			...
			response.Code = 40001
			response.Msg = fmt.Sprintf("参数解析失败：%v", err)
			response.Data = iris.NewProblem().Title("Validation error").Detail("One or more fields failed to be validated").Type("/api/v1/addresss/validation-errors").Key("errors", validationErrors)
